...
bool gx_gui_open_display(gx_epicUI *ui) {
	return true; // STUB
}

void gx_gui_create_window_and_surface(gx_epicUI *ui) {
	// prepare window class
	static TCHAR szClassName[] = TEXT("gx_epicUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_register_controller_message(gx_epicUI *ui) {
	// dummy, not required on MSWin
}

void gx_gui_destroy_main_window(gx_epicUI *ui) {
	DestroyWindow(ui->win);
	// safe to use: doesnt unregister if there are still windows of this class
	UnregisterClass(TEXT("gx_epicUIClass"), NULL);
}

void gx_gui_resize_surface(gx_epicUI *ui) {
	RECT rect;
...
void gx_gui_send_controller_event(gx_epicUI *ui, int controller) {
...
void event_handler(gx_epicUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_epicUI *ui = (gx_epicUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
